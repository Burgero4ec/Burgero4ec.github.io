...
async function fetchSeasonData() {
  try { return await Promise.all([apiGet('/api/season'), apiGet('/api/countries')]); }
    ...
    const seasonData = await fetchJson('seasoninfo.json')
      .catch(() => fetchJson('data/seasons/season-26.json'))
      .catch(() => fetchJson('data/seasons/season-25.json'))
      .catch(() => ({}));
    const countriesData = await fetchJson('countries2014.json').catch(() => ({}));
